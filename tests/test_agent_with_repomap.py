...
def test_agent_initialization_with_repomap(mock_llm, tmp_path):
    ...
    agent = CoderAgent(mock_llm, tools=ALL_TOOLS, use_repo_map=True, repo_root=str(tmp_path))
...
def test_agent_without_repomap(mock_llm, tmp_path):
    ...
    agent = CoderAgent(mock_llm, tools=ALL_TOOLS, use_repo_map=False, repo_root=str(tmp_path))
