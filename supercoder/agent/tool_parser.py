...
class BaseToolParser(ABC):
    ...
    def try_parse_all(self, text: str) -> list[ToolCall]:
        """Parse ALL tool calls from text. Default falls back to try_parse."""
...
class QwenStyleParser(BaseToolParser):
    ...
    _name_pattern_simple = re.compile(r"to=(?:tool[:\s.])?([a-zA-Z0-9_-]+)\s*(\{)", re.IGNORECASE)
    ...
    def try_parse(self, text: str) -> ToolCall | None:
        for pattern in (
            self._name_pattern_simple,
            self._name_pattern_full,
            self._name_pattern_channel,
        ):
...
class JsonBlockParser(BaseToolParser):
    ...
    def try_parse(self, text: str) -> ToolCall | None:
        ...
        try:
            ...
            return ToolCall(
                name=name, arguments=args, raw_match=match.group(0), format_name=self.name
            )
        except (json.JSONDecodeError, KeyError):
            return None
...
class XmlFunctionParser(BaseToolParser):
    ...
    name = "xml_function"
    pattern = re.compile(
        r'<function_call\s+name=["\']([^"\']+)["\']>(.*?)</function_call>', re.DOTALL
    )
    ...
    def try_parse(self, text: str) -> ToolCall | None:
        ...
        return ToolCall(name=name, arguments=args, raw_match=match.group(0), format_name=self.name)
...
class GlmToolCallParser(BaseToolParser):
    ...
    def try_parse(self, text: str) -> ToolCall | None:
        ...
        return ToolCall(
            name=tool_name, arguments=args, raw_match=match.group(0), format_name=self.name
        )
