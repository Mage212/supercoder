...
- **Secret scrubber covers modern key formats (M2)**: `sk-proj-…` (OpenAI, since 2024) and `sk-ant-api03-…` (Anthropic) are now redacted in bare form (tracebacks, command output), not only when preceded by a `key=` field name. Also Google AI (`AIza…`) and Replicate (`r8_…`).
- **Trust gate covers project rules and sessions (R2-2, R2-7)**: an independent audit found that `.supercoder/rules/*.md` (injected into the system prompt as mandatory instructions) and `.supercoder/sessions/*.json` (replayed verbatim into the model context on `/continue`) were NOT covered by the C1/C2 trust gate — both are prompt-injection vectors from a cloned malicious repo. Both are now gated behind the same per-repo trust decision (`allow_project_rules`, `allow_session_load`).
- **Trust resolution separates config and permission axes (R2-1 / BYPASS A)**: the original C1/C2 gate used a single `repo_trusted` flag that defaulted to True when no `permissions.yaml` existed, which silently honored a planted `.supercoder.yaml` endpoint redirect with no prompt. Config and permission trust are now independent axes (`config_trusted` / `perms_trusted`), each gated by the trust store or an explicit interactive grant. Pure `resolve_repo_trust()` is unit-tested via a decision table.
- **Trust store rejects newline-in-path injection (R2-4)**: a repo path containing a newline would write two lines into the newline-delimited trust store, silently pre-trusting a second arbitrary path. `_normalize` now rejects paths containing `\n`/`\r`.
- **Subprocess env sanitization covers `*_KEY` (R2-5)**: `STRIPE_SECRET_KEY`, `SIGNING_KEY`, `ENCRYPTION_KEY`, `PRIVATE_KEY` and other `*_KEY`-suffixed names (without `_API_`) are now stripped, closing a leak where `env`/`printenv` could exfiltrate them.

### Correctness

- **CRLF files preserved on edits (M4)**: `code-edit` read files with universal-newline translation (silently rewriting `\r\n` → `\n`) and computed whitespace/fuzzy match offsets with a `+1`-per-line heuristic that dropped the `\r`. Both are fixed (`open(newline="")` + `splitlines(keepends=True)`), so CRLF files keep their line endings and adjacent lines are no longer corrupted on non-exact matches. The fix now covers ALL edit operations (search_replace, insert_after, insert_before, replace_lines, append) via a shared `_read_for_edit` / `_join_lines` helper — the original fix only patched search_replace, leaving the other four silently flattening CRLF.
